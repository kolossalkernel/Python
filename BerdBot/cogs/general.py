...
class General(commands.Cog):
    ...
    async def help(self, ctx):
        if await self.channel_check(ctx) is True:
            cmds = ["```!help - Shows list of commands",
                    ...
                    "\n!play - Plays the specified YouTube url audio"
                    "\nExample usage: !play https://youtu.be/dQw4w9WgXcQ"
                    "\nNote: Playlists not supported yet, may lag the bot",
    ...
    async def _8ball(self, ctx, *, question):
        if await self.channel_check(ctx) is True:
            ...
            firstword = "want wanna who what whats what's wat wats when where why would " \
            ...
            # Answers
            if question.lower().startswith("will andy pick me") or question.lower().startswith("will andy pick me?"):
    ...
    async def roulette(self, ctx):
        if await self.channel_check(ctx) is True:
            ...
            if random.choice(responses) == "BANG":
                await ctx.send(file=discord.File("C:/Users/You/Documents/MEGA/PyCharm/Projects/BerdBot/png/BANG.png"))
                async for member in ctx.guild.fetch_members(limit=None):
                    if ctx.author.id == member.id:
                        link = await ctx.channel.create_invite()
                        await user.send("The shadow of Death has fallen upon you...\nYou were shot in an unlucky game of roulette.\n\n**Click this link to be revived:**\n" + str(link))
                        await member.kick()
            else:
                await ctx.send(file=discord.File("C:/Users/You/Documents/MEGA/PyCharm/Projects/BerdBot/png/click.png"))
        else:
            await ctx.send(f"Hey dummy put that command in the <#843892871396720700> channel.")
