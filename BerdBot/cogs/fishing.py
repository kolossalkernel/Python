...
class Fishing(commands.Cog):
    ...
    async def cast_error(self, ctx, error):
        if isinstance(error, commands.CommandOnCooldown):
            ...
            minutes = seconds // 60
            seconds %= 60
            await ctx.send(f"That's enough fishing for now.\nTry again in {minutes} minutes and {seconds} seconds...")


    # Junk catches
    async def cast_junk(self, ctx, users):
        junk_names = ["Broken Pot", "Old Shoe", "Fishing Rod", "Tin Can", "Plastic Bottle", "Plastic Bag", "Rusty Pipe", "Crusty Jeans", "Broken Lock", "Novelty Fish", "Toy Car", "Cardboard Box", "copy of E.T. the Extra-Terrestrial", "Rubber Tire"]
        junk_class = 7
        junk_current = random.choice(junk_names)
        with open("C:/Users/You/Documents/MEGA/PyCharm/Projects/BerdBot/json/fishing.json", "w") as f:
            json.dump(users, f, indent=4)
        await ctx.send(
            f"**You caught a** ***{junk_current}!...***\nClass: Junk")


    # Small fish
    async def cast_small(self, ctx, users):
        if random.randint(1, 3) > 1:
            small_fish_names = ["Anchovy", "Crab", "Lobster", "Snail", "Smallmouth Bass", "Steelhead Trout", "Yellow Perch", "Goldfish", "Chub", "Sea Horse", "Bluegill", "Black Crappie", "Clownfish", "Oscar", "Yellow Croaker", "Lionfish"]
            ...
            await self.assign_last(ctx, users, small_fish_weight_pounds, small_fish_weight_ounces, small_fish_length_feet, small_fish_length_inches, small_fish_class, small_fish_current)
            users[str(ctx.author.id)]["total_fish"] += 1
            if users[str(ctx.author.id)]["last_fish_class"] < users[str(ctx.author.id)]["best_fish_class"]:
                await self.assign_best(ctx, users,  small_fish_weight_pounds, small_fish_weight_ounces, small_fish_length_feet, small_fish_length_inches, small_fish_class, small_fish_current)
            elif users[str(ctx.author.id)]["last_fish_class"] == users[str(ctx.author.id)]["best_fish_class"]:
                if users[str(ctx.author.id)]["last_fish_pounds"] > users[str(ctx.author.id)]["best_fish_pounds"]:
                    await self.assign_best(ctx, users, small_fish_weight_pounds, small_fish_weight_ounces, small_fish_length_feet, small_fish_length_inches, small_fish_class, small_fish_current)
                elif users[str(ctx.author.id)]["last_fish_pounds"] == users[str(ctx.author.id)]["best_fish_pounds"]:
                    if users[str(ctx.author.id)]["last_fish_ounces"] > users[str(ctx.author.id)]["best_fish_ounces"]:
                        await self.assign_best(ctx, users, small_fish_weight_pounds, small_fish_weight_ounces, small_fish_length_feet, small_fish_length_inches, small_fish_class, small_fish_current)
    ...
    async def cast_medium(self, ctx, users):
        if random.randint(1, 3) > 1:
            medium_fish_names = ["Blobfish", "Paddlefish", "Sea Bass", "Walleye", "Muskellunge", "Catfish", "Snapper", "Pink Salmon", "Albino Carp", "Cutthroat Trout", "Tautog", "Blackfish", "Dogfish", "Rockfish", "Needlefish", "Northern Pike", "Anglerfish", "Electric Eel", "Albacore", "Turbot", "Burbot", "Lumpfish", "Haddock", "Chum Salmon"]
            ...
            await self.assign_last(ctx, users, medium_fish_weight_pounds, medium_fish_weight_ounces, medium_fish_length_feet, medium_fish_length_inches, medium_fish_class, medium_fish_current)
            users[str(ctx.author.id)]["total_fish"] += 1
            if users[str(ctx.author.id)]["last_fish_class"] < users[str(ctx.author.id)]["best_fish_class"]:
                await self.assign_best(ctx, users, medium_fish_weight_pounds, medium_fish_weight_ounces, medium_fish_length_feet, medium_fish_length_inches, medium_fish_class, medium_fish_current)
            elif users[str(ctx.author.id)]["last_fish_class"] == users[str(ctx.author.id)]["best_fish_class"]:
                if users[str(ctx.author.id)]["last_fish_pounds"] > users[str(ctx.author.id)]["best_fish_pounds"]:
                    await self.assign_best(ctx, users, medium_fish_weight_pounds, medium_fish_weight_ounces, medium_fish_length_feet, medium_fish_length_inches, medium_fish_class, medium_fish_current)
                elif users[str(ctx.author.id)]["last_fish_pounds"] == users[str(ctx.author.id)]["best_fish_pounds"]:
    ...
    async def cast_large(self, ctx, users):
        if random.randint(1, 3) > 1:
            large_fish_names = ["King Salmon", "Cod", "Dolphin", "Sturgeon", "Sea Turtle", "Bull Shark", "Great Barracuda", "Leerfish", "Red Drum", "Dogtooth Tuna", "Cobia", "Mackerel", "Monkfish", "Toothfish", "Sablefish"]
            ...
            await self.assign_last(ctx, users, large_fish_weight_pounds, large_fish_weight_ounces, large_fish_length_feet, large_fish_length_inches, large_fish_class, large_fish_current)
            users[str(ctx.author.id)]["total_fish"] += 1
            if users[str(ctx.author.id)]["last_fish_class"] < users[str(ctx.author.id)]["best_fish_class"]:
                await self.assign_best(ctx, users, large_fish_weight_pounds, large_fish_weight_ounces, large_fish_length_feet, large_fish_length_inches, large_fish_class, large_fish_current)
            elif users[str(ctx.author.id)]["last_fish_class"] == users[str(ctx.author.id)]["best_fish_class"]:
                if users[str(ctx.author.id)]["last_fish_pounds"] > users[str(ctx.author.id)]["best_fish_pounds"]:
                    await self.assign_best(ctx, users, large_fish_weight_pounds, large_fish_weight_ounces, large_fish_length_feet, large_fish_length_inches, large_fish_class, large_fish_current)
                elif users[str(ctx.author.id)]["last_fish_pounds"] == users[str(ctx.author.id)]["best_fish_pounds"]:
    ...
    async def cast_grand(self, ctx, users):
        if random.randint(1, 3) > 1:
            grand_fish_names = ["Goliath Grouper", "Black Marlin", "Bluefin Tuna", "Sawfish", "Halibut"]
            ...
            await self.assign_last(ctx, users, grand_fish_weight_pounds, grand_fish_weight_ounces, grand_fish_length_feet, grand_fish_length_inches, grand_fish_class, grand_fish_current)
            users[str(ctx.author.id)]["total_fish"] += 1
            if users[str(ctx.author.id)]["last_fish_class"] < users[str(ctx.author.id)]["best_fish_class"]:
                await self.assign_best(ctx, users, grand_fish_weight_pounds, grand_fish_weight_ounces, grand_fish_length_feet, grand_fish_length_inches, grand_fish_class, grand_fish_current)
            elif users[str(ctx.author.id)]["last_fish_class"] == users[str(ctx.author.id)]["best_fish_class"]:
                if users[str(ctx.author.id)]["last_fish_pounds"] > users[str(ctx.author.id)]["best_fish_pounds"]:
                    await self.assign_best(ctx, users, grand_fish_weight_pounds, grand_fish_weight_ounces, grand_fish_length_feet, grand_fish_length_inches, grand_fish_class, grand_fish_current)
                elif users[str(ctx.author.id)]["last_fish_pounds"] == users[str(ctx.author.id)]["best_fish_pounds"]:
    ...
    # Assign last fish
    async def assign_last(self, ctx, users, fish_weight_pounds, fish_weight_ounces, fish_length_feet, fish_length_inches, fish_class, fish_current):
    ...
    # Assign best fish
    async def assign_best(self, ctx, users, fish_weight_pounds, fish_weight_ounces, fish_length_feet, fish_length_inches, fish_class, fish_current):
    ...
    async def cast_break(self, ctx):
        if ctx.message.guild.id == 735601308597485578:  # respecc regg
            await ctx.send("Something broke the line... " + "<:monkaS:795881948689334292>")
        elif ctx.message.guild.id == 696557815959126126:  # Nick's Cursed E-House
            await ctx.send("Something broke the line... " + "<:monkaS:820364703083528213>")
    ...
    async def pb(self, ctx):
        if await self.channel_check(ctx) is True:
            ...
            if ctx.author.nick == None:
                em = discord.Embed(title=f"{ctx.author.name}'s Best Catch:", color=discord.Color.blue())
                em.add_field(name=f"Fish:", value=fish_name, inline = False)
                em.add_field(name=f"Weight:", value=f"{fish_weight_pounds} pounds and {fish_weight_ounces} ounces", inline = False)
                em.add_field(name=f"Size:", value=f"{fish_length_feet} feet and {fish_length_inches} inches", inline = False)
                em.add_field(name=f"Class:", value=new_class, inline = False)
            else:
                em = discord.Embed(title=f"{ctx.author.nick}'s Best Catch:", color=discord.Color.blue())
                em.add_field(name=f"Fish:", value=fish_name, inline = False)
                em.add_field(name=f"Weight:", value=f"{fish_weight_pounds} pounds and {fish_weight_ounces} ounces", inline = False)
                em.add_field(name=f"Size:", value=f"{fish_length_feet} feet and {fish_length_inches} inches", inline = False)
                em.add_field(name=f"Class:", value=new_class, inline = False)
    ...
    async def last(self, ctx):
        if await self.channel_check(ctx) is True:
            ...
            if ctx.author.nick == None:
                em = discord.Embed(title=f"{ctx.author.name}'s Last Catch:", color=discord.Color.blue())
                em.add_field(name=f"Fish:", value=fish_name, inline=False)
                em.add_field(name=f"Weight:", value=f"{fish_weight_pounds} pounds and {fish_weight_ounces} ounces",
                             inline=False)
                em.add_field(name=f"Size:", value=f"{fish_length_feet} feet and {fish_length_inches} inches", inline=False)
                em.add_field(name=f"Class:", value=new_class, inline=False)
            else:
                em = discord.Embed(title=f"{ctx.author.nick}'s Last Catch:", color=discord.Color.blue())
    ...
    async def classes(self, ctx):
        if await self.channel_check(ctx) is True:
            cmds = ["```Junk: Garbage lying deep within the ocean. Maybe something interesting is down there.",
    ...
    async def best(self, ctx):
        if await self.channel_check(ctx) is True:
            ...
            em = discord.Embed(title="Best Fish Ever Caught:", color=discord.Color.blue())
            em.add_field(name=f"Fisher:", value=user_name, inline=False)
            em.add_field(name=f"Fish:", value=best_name, inline=False)
            em.add_field(name=f"Weight:",
                         value=f"{best_pounds} pounds and {best_ounces} ounces", inline=False)
            em.add_field(name=f"Size:", value=f"{best_feet} feet and {best_inches} inches",
                         inline=False)
            em.add_field(name=f"Class:", value=new_class, inline=False)
            await ctx.send(embed=em)
        else:
    ...
    async def get_fishing_data(self):
        with open("C:/Users/You/Documents/MEGA/PyCharm/Projects/BerdBot/json/fishing.json", "r") as f:
            users = json.load(f)
        return users
